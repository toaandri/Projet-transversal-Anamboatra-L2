...
/** Retire espaces + BOM UTF-8 (souvent ajouté par le Bloc-notes Windows sur .env). */
...
const env = {
  ...
  /**
   * CDC v2.2 — jeton de la console d'administration technique (hors UML).
   * Toute requête /api/admin/* doit fournir l'en-tête X-Admin-Token === cette valeur.
   */
  adminSetupToken: trimOr(process.env.ADMIN_SETUP_TOKEN, 'dev-admin-token-change-me'),
  /** Si défini, remplace host/port/user/password/database (URL Sequelize). */
  databaseUrl,
  postgres: {
